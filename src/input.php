...
class Input {
    ...
    function getAttributes() {
        ...
        foreach ($this->element->attributes as $attr) {
            $name = mb_strtolower($attr->nodeName);
            $value = mb_strtolower($attr->nodeValue);
    ...
    private function filterVar($value, $filter) {
        if (empty($element['value'])) {
            throw new softError(_f("No value"));
        }
        if (filter_var($element['value'], $filter) === false) {
            throw new hardError(_("Invalid value"));
        }
        return true;
    }
    ...
    private function validateAccept($value) {
        if ($this->getType() != 'file') {
            throw new softError(_f("Input type doesn't support this attribute", $this->getType()));
        }
    }
    ...
    private function validatePattern($value) {
        ...
        if (empty($this->getAttribute('pattern'))) {
            throw new softError(_f("No %s attribute value", "pattern"));
        }
        if (filter_var($value, FILTER_VALIDATE_REGEXP, ["options" => ["regexp" => "/^" . $this->getAttribute('pattern') . "$/"]]) === false) {
            throw new hardError(_("Value doesn't match pattern"));
        }
        return true;
    }
    ...
    private function validateRequired($value) {
        ...
        if (empty($value)) {
            throw new hardError(_("Input is required"));
						return false;
        }
				return true;
    }
     ...
     */
		private function validateStep($value){
			...
			if(!in_array($this->getAttribute('type'),['date','month', 'week','datetime-local','range'])){
				throw new softError(_f("Input type doens't support this attributes",$this->getType()));
			}
		}
		  /**
     * Validate the 'required' attribute for specific input types.
     ...
     */
		private function validateMinlength($value){
			...
			if(mb_strlen($value) < $this->getAttribute('minlength')){
				throw new hardError(_('Value is shorter than minimum required length'));
				return false;
			}
		}
		  /**
     ...
     */
		private function validateMaxlength($value){
			...
			if(mb_strlen($value) > $this->getAttribute('maxlength')){
				throw new hardError(_('Value is longer than the maximum permitted length'));
			}
		}
		private function validateFile($value){
			switch($value['error']){
				case UPLOAD_ERR_INI_SIZE: 	
					throw new hardError(_('File exceeds max size in php.ini'));		
					return false;
					break;
				case UPLOAD_ERR_PARTIAL:		
					throw new hardError(_('File exceeds max size in html form'));		
					return false;
					break;
				case UPLOAD_ERR_NO_FILE: 		
					throw new hardError(_('File No file was uploaded'));						
					return false;
					break;
				case UPLOAD_ERR_NO_TMP_DIR:	
					throw new hardError(_('No /tmp dir to write to'));
					return false;
					break;
				case UPLOAD_ERR_CANT_WRITE:	
					throw new hardError(_('File:: Error writing to disk'));
					return false;
					break;
				default:
				return true;
